...
const Home = () => {
          ...
          <meter
            className={`${
              uvIndex <= 2
                ...
                ? " bg-green-600/90"
                : ""
            } -rotate-90 w-18  h-7 uvIndex overflow-hidden`}
            min="0"
            max="11"
            low="3"
            high="6"
    ...
    },
    {
      ...
      pic: (
        <p>
          <FaCloud
            className={`size-16 ${
              theme === true ? "text-blue-300" : "text-zinc-100"
            }`}
          />
        </p>
      ),
    ...
    },
    {
      ...
      pic: (
        <p>
          <IoTimer
            className={`size-16 ${
              theme === true ? "text-zinc-500" : "text-zinc-100"
            }`}
          />
        </p>
      ),
      ...
      pic: (
        <p>
          <FiNavigation2
            ...
            style={{
              transform: `rotate(${weather.data.values.windDirection}deg)`,
            }}
          />
        </p>
      ),
      ...
      pic: (
        <p>
          <WiRainMix
            ...
            style={{ transform: `rotate(${weather.location.name}deg)` }}
          />
        </p>
      ),
  ...
  const hideToggle = (id) => {
    setHideWeight((prev) => ({ ...prev, [id]: !prev[id] }));
  };
  ...
  return (
    ...
    >
      {weather ? (
        <div className=" flex  flex-col w-full h-full overflow-x-scroll p-5 pb-28 text-white">
          ...
          >
            <RiMenuUnfoldFill
              ...
              className={` sideBerAndLocationTrack size-11 border-1 border-black/20 p-2 rounded-md ${
                theme === true ? "text-violet-700" : "text-zinc-300"
            ...
            >
              <h1 className="flex gap-0.5 font-bold text-white/90 text-md">
          ...
          <div
            className={`flex w-full gap-5 overflow-scroll h-34 pt-2 pb-2 pl-1 border-t ${
              theme === true ? "border-violet-500" : "border-zinc-300"
            }  lg:h-40`}
          >
   ...
   border min-w-18 h-full rounded-2xl ${
     theme === true ? "text-violet-500" : "text-zinc-300"
   } p-1 shadow shadow-black/40 lg:min-w-20 `}
              >
                <div className="text-sm flex flex-col items-center">
                  {weekday ===
                  new Date(timeline.time).toLocaleDateString("en-US", {
                    weekday: "long",
                  })
                    ? "Today"
                    : new Date(timeline.time).toLocaleDateString("en-US", {
                  ...
                  <div className="">
                    {new Date(timeline.time).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                      hour12: "true",
                    })}
                ...
                />
                <div className="">
                  {timeline.values.temperatureApparent.toFixed()}°C
                </div>
              </div>
            ))}
          </div>
          ...
          <Menubar
            ...
            setSearchPage={setSearchPage}
            searchPage={searchPage}
            homepageForecast={homepageForecast}
            setHomepageForecast={setHomepageForecast}
            menuItem={todayLine}
            hideToggle={hideToggle}
            hideWeight={hideWeight}
          />
        ...
        <p className="flex w-full h-screen justify-center items-center text-5xl text-zinc-950">
