...
const [selectDay,setSelectDay] = useState(fitDFW) ;
  ...
  return (
    <div className={`w-full h-full ${theme===true? "bg-violet-100" : "bg-zinc-800"} overflow-hidden pb-20`} >
...
<div className="flex relative flex-col w-full h-full overflow-x-scroll p-5  text-white">
 ...
 <div className={`flex w-full header fixed top-0 pt-3 pb-1 flex-row z-2 ${theme===true? "bg-violet-100 headerLight" : "bg-zinc-800 headerDark"} pl-5 pr-5 justify-between  items-center`}>
  <NavLink to={"/"}>
        <IoChevronBack className={`sideBerAndLocationTrack size-11 border-1 border-black/20 p-2 rounded-md ${theme===true? "text-violet-700" : "text-zinc-100"} hover:scale-105 cursor-pointer`}/></NavLink>
...
<ForecastMain selectDay={selectDay} setSelectDay={setSelectDay}/>
 <div className={`flex flex-row w-fit h-7 ${theme===true? "bg-violet-500":"bg-zinc-600"} rounded-tr-[2rem] pl-8 pr-8 rounded-tl-2xl ml-0.5 mt-6 text-xl lg:absolute lg:right-[51.3%] lg:top-[20%]`}>Highlights</div>
