 ...
 const handleWind = () => {
  ...
  return (<>   
   <div className="flex flex-row justify-between items-center mt-16 ">
    ...
    <h1 className={`boxShadow2 flex relative z-1 flex-row justify-center items-center text-xl font-bold text-white/90 pl-5 pr-5 pb-2 pt-2 rounded-b-[50%] rounded-t-[50%] outline-none ${theme===true? "bg-gradient-to-b from-violet-100 via-violet-600 to-violet-100" : "bg-zinc-700"}`}>Hourly details</h1>
...
<div className={`mt-5 p-2 rounded-t-2xl flex w-full h-fit justify-around ${theme===true? " bg-violet-500 " : " bg-gradient-to-bl from-zinc-500 to-zinc-700"}`}>
  <div onClick={handleHumidity} className={`${humidity===true? " border-2 " : ""} ${theme===true? " bg-violet-500 " : " bg-zinc-700"}  pl-2 pr-2 rounded-full flex justify-center items-center p-1`}>Humidity</div>
  <div onClick={handleWind} className={`${wind===true? " border-2 " : ""} ${theme===true? " bg-violet-500 " : " bg-zinc-700"}  pl-2 pr-2 rounded-full flex justify-center items-center p-1`}>Wind</div>    
