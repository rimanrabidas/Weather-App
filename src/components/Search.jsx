 ...
 const searchList = [
  ...
  return (
    <div className={`flex ${searchPage === false ? "w-0  h-0  top-10 left-[50%]" : "w-full  h-[132%] "} transition-all duration-100 overflow-hidden  fixed  z-2 flex-col items-center  top-0 left-0 right-0 ${theme===true? "bg-violet-100 lg:bg-black/50" : "lg:bg-white/50 bg-zinc-800"} `}>
        ...
        <div className={`${!searchPage? "hidden" : "block"} ${theme===true? "bg-violet-100 headerLight" : "bg-zinc-800 headerDark"} w-full h-fit pb-1 pt-3 pr-5 pl-5 fixed top-0 right-0 flex flex-row justify-between lg:justify-evenly`}>
        <IoChevronBack onClick={() => setSearchPage(!searchPage)} className={` sideBerAndLocationTrack size-11 border-1 border-black/20 p-2 rounded-md  ${theme===true? "text-violet-700 " : "text-zinc-200 "} hover:scale-105 cursor-pointe`} />
             <BiCurrentLocation onClick={handleGeolocation} className={` sideBerAndLocationTrack size-11 border-1  border-black/20 p-2 rounded-md ${theme===true? "text-violet-700 " : "text-zinc-200 "} hover:scale-105 cursor-pointer `}/>
             </div>
...
<div className="flex flex-col items-center min-h-55 max-h-88 pb-44 overflow-x-scroll mt-4 w-[90%] border-l border-white/80 border-r ">
        ...
        <div onClick={() => setSearchPage(!searchPage)} className={`${!searchPage? "hidden" : "block"} flex justify-center items-center w-22 h-9 rounded-full fixed bottom-8 ${theme===true? "bg-violet-500 " : "bg-zinc-200 "} shadow-lg shadow-black/40 hover:scale-105 cursor-pointer`}>
        <IoClose className={`size-12 ${theme===true? "text-white/90 " : "text-black/80 "}`}/>
        </div>
        </div>  
    </div>
  )
