...
const Search = ({searchpage,setsearchpage}) => {
 ...
 const {inputCity,setInputCity,handleSearch,theame,setCity,History,setHistory} = UseApi();
...
heading: "Berlin",
  ...
  return (
    <div className={`flex ${searchpage === false ? "hidden" : "block"}  fixed  z-2 flex-col items-center  top-0 left-0 right-0 ${theame===true? "bg-violet-100 lg:bg-black/50" : "lg:bg-white/50 bg-zinc-800"} w-full  h-[132%]`}>
        <div className={`flex  flex-col w-full h-full items-center ${theame===true? "lg:bg-violet-100" : " lg:bg-zinc-800"} lg:w-[40%] lg:left-[30%] `}>

        <div className={` ${theame===true? "bg-violet-100 headerlight" : "bg-zinc-800 headerdark"} w-full h-fit pb-1 pt-3 pr-5 pl-5 fixed top-0 right-0 flex flex-row justify-between lg:justify-evenly`}>
        ...
        <div className="w-[90%] h-12 mt-18 bg-white/80 flex justify-between items-center rounded-full">
         ...
         <button onClick={handleSearch} ><IoMdSearch className={`size-10  rounded-full p-2 mr-1  ${theame===true? "bg-violet-600 " : "bg-zinc-800 "} hover:scale-105 cursor-pointer`} /></button>  
        </div>
        <div onClick={() => setHistory([])} className={`flex flex-row w-[90%] min-h-10 h-fit mt-4 justify-between overflow-hidden  ${theame===true? "bg-red-400 " : "bg-black/80 "} rounded-2xl `}>
        ...
        History.map((curHistory,index)=>(
            <div key={index} className={`flex flex-row m-2 p-1 w-[95%] rounded-xl ${theame===true? "bg-violet-100 " : "bg-zinc-800 "} text-sm h-full justify-between items-center`}>
...
<div className="flex flex-col items-center min-h-55 max-h-88 pb-44 overflow-x-scroll mt-4 w-[90%] border-l border-white/80 border-r ">
        { searchlist.map(({id,heading,img}) => (
             <div key={id} onClick={() =>setCity(heading)} className={`flex flex-row w-[90%] mb-4 shadow-sm  h-fit ${theame===true? "bg-white/80 shadow-violet-600 text-black/90" : "bg-white/20 shadow-black text-white/80 "} justify-between rounded-full items-center`}>
