...
import { FaAngleDown, FaAngleUp, FaArrowTrendUp, FaLinkedin, FaSquareGithub } from 'react-icons/fa6';
...
const [weightBer,setWeightBer] = useState(true)
  return (
    <div  className= {` ${menubar === false ? "w-0 h-0 fixed top-7 left-10" : "w-full h-full fixed"} overflow-hidden transition-all duration-100 top-0 left-0 z-50   bg-black/70`} >
        ...
        <FiMinimize2 onClick={() => setMenubar(!menubar)} className={` sideBerAndLocationTrack size-11 border-1 border-black/20 p-2 rounded-md ${theme===true? "text-violet-700" : "text-zinc-200"} hover:scale-105 cursor-pointer absolute top-2 right-2`} />

<div onClick={() => setTheme(!theme)} className="ThemeIcon flex justify-center items-center size-11 border-1 border-black/20 p-2 rounded-md hover:scale-105 cursor-pointer absolute top-2 left-2">
...
{theme===false?
 ...
 <IoMoon className=' ThemeChange size-12 text-zinc-900'/>
       ...
       <div className={` flex flex-col gap-4 w-full h-[75%] mt-13 p-2 ${theme===true? "text-black/80" : "text-white/80"}`}>
       <div className="w-full h-[1px] bg-zinc-300"></div>

<NavLink to={"/"} onClick={() => setMenubar(!menubar)} className="flex w-full h-fit text-xl gap-5 items-center"><TiHome className='size-8'/>  Home</NavLink>
...
<NavLink to={"/forecast"} className="flex w-full h-fit text-xl gap-5 items-center"><FaArrowTrendUp className='size-8'/>  Forecast</NavLink>
        ...
        <div className={`flex w-full h-fit items-center flex-row gap-5  `}><IoMdMoon className='size-8 ' /><p className='text-xl'>Theme </p>
        <button onClick={() => setTheme(!theme)} className={`w-12 h-5 rounded-full  menuShadow ${theme===true? "bg-black" : "bg-amber-400"} flex items-center`}> <div className={`w-6 h-4  rounded-full  ${theme===true? "bg-white ml-0.5" : "ml-5.5 bg-black/80"} `}></div></button>
          </div>
          <div className="flex w-full h-fit items-center flex-row gap-5  "><p>Hourly Forecast On Homepage</p>
          <button onClick={() => setHomepageForecast(!homepageForecast)} className={`w-12 h-5 rounded-full  menuShadow ${homepageForecast===false? "bg-black" : "bg-violet-500"} flex items-center`}> <div className={`w-6 h-4  rounded-full  ${homepageForecast===false? "bg-white ml-0.5" : "ml-5.5 bg-black/80"} `}></div></button>
          </div>
...
{todayLine.map((index) => (
       ...
       <div className={`flex flex-col w-full h-20 items-center justify-center gap-2 ${theme===true? "text-black/80" : "text-white/80"}`}>
        ...
        <FaTelegramPlane className={`size-6 ${theme===true? "text-violet-100 bg-black/80" : "bg-white/80 text-black/80"}  rounded-sm p-[2px]`} />
        </div>
       </div>
