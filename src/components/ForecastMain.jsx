...
const ForecastMain = () => {
 ...
 const {weather,localTimeDate,
    localTimeMonth,localWeekDay,time} = UseApi();

  return (
    <div className=""> 
      <div className="flex flex-row justify-between items-center mt-5 ">
    <div className="size-8"><GiNinjaStar className='chakra size-8 text-violet-600  '/></div>
    <h1 className="boxshadow2 flex relative z-1 flex-row justify-center items-center text-xl font-bold text-white/90 pl-5 pr-5 pb-2 pt-2 rounded-b-[50%] rounded-t-[50%] outline-none bg-gradient-to-b from-violet-100 via-violet-600 to-violet-100"> 5 Day Forecast</h1>
    ...
    <div className=" mainpart relative flex flex-col w-full min-h-50 max-h-55 h-fit mt-5 rounded-3xl  bg-gradient-to-bl from-violet-400 to-violet-600 hover:scale-101 cursor-pointer">
    ...
    <h1 className='temp flex text-7xl pl-3 font-bold bg-gradient-to-b from-white/100 to-violet-500 bg-clip-text text-transparent '>{weather.main.temp.toFixed()} <p className='text-3xl font-bold bg-gradient-to-b from-white/100 to-violet-500 bg-clip-text text-transparent'> °C</p></h1>
