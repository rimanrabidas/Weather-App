  ...
  return (
    <div className="">
      ...
      <div className=" w-full relative text-violet-500 h-34 mt-5 flex gap-5 overflow-y-scroll pl-1 lg:w-fit">
        {dailyForecastWeather.data.timelines[0].intervals
          .slice(-5)
          .map((id, index) => (
            <div
              onClick={() => setSelectDay(id)}
              key={index}
              className={`flex flex-col justify-between items-center p-2 border  min-w-18 h-[90%] rounded-2xl ${
                theme === true ? "text-violet-600" : "text-zinc-100"
              } ${
                selectDay?.startTime === id.startTime
                  ? `${
                      theme === true
                        ? "bg-gradient-to-t from-violet-500 via-orange-50/5 to-violet-600  text-white "
                        : "bg-gradient-to-t from-zinc-500 via-orange-50/5 to-zinc-600  text-white "
                    }`
                  : "text-violet-500"
              } `}
            >
              <div className="">
                {index === 0
                  ? "Tomorrow"
                  : new Date(id.startTime).toLocaleDateString("en-US", {
                      weekday: "long",
                    })}
              </div>
              <img
                className="size-10"
                src={`/${id.values.weatherCode}.png`}
                alt="Weather Icon"
              />
      ...
      {selectDay && (
        <div
          className={` mainPart relative flex flex-col w-full lg:w-110 min-h-50 max-h-62 lg:h-62 h-fit pb-4 mt-2 rounded-3xl ${
            theme === true
        ...
        >
          <div className="flex flex-row justify-between text-white pt-4 pl-4 ">
            <h1 className="text-xl font-medium">
              {new Date(selectDay.startTime).toLocaleDateString()}
              <div className="text-sm mb-5">
                {new Date(selectDay.startTime).toLocaleDateString("en-US", {
                  weekday: "long",
                })}
              </div>
            </h1>
            <h1 className="flex flex-col items-center text-2xl font-medium pr-4">
              {selectDay.startTime ===
              dailyForecastWeather.data.timelines[0].intervals[
                dailyForecastWeather.data.timelines[0].intervals.length - 5
              ].startTime
                ? "Tomorrow"
                : new Date(selectDay.startTime).toLocaleDateString("en-US", {
                    weekday: "long",
              ...
              <p className="text-sm">
                {new Date(selectDay.startTime).toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                  hour12: "true",
                })}
          ...
          <div className="flex flex-col">
            <h1
              className={`temp flex text-7xl pl-3 font-bold ${
                theme === true
                  ? "bg-gradient-to-b  from-white/100 to-violet-400"
                  : "bg-gradient-to-b  from-zinc-50 to-zinc-200"
              } bg-clip-text text-transparent `}
            >
              {selectDay.values.temperature.toFixed()}{" "}
              <p
                className={`text-3xl font-bold ${
                  theme === true
                    ? "bg-gradient-to-b  from-white/100 to-violet-500"
                    : "bg-gradient-to-b  from-zinc-50 to-zinc-200"
                } bg-clip-text text-transparent`}
              >
                {" "}
            ...
            <div className="flex items-end gap-5 font-bold text-2xl pl-4 pt-1 text-white/80">
              ...
              <div className="font-light">
                <div className="text-sm flex items-center gap-1">
                  <FaTemperatureHalf />
                  Feels like : {selectDay.values.temperatureApparent.toFixed()}
                  °C
                </div>
                <div className="text-sm flex items-center gap-1">
                  <IoIosEye />
                  Visibility : {selectDay.values.visibility.toFixed()} km
                </div>
              </div>
            </div>
          </div>
          <div className="SunnyCloudy">
            <img
              className="absolute right-4 bottom-0 size-35"
              src={`/${selectDay.values.weatherCode}.png`}
              alt="Weather Icon"
            />
          </div>
        </div>
