...
const [hideIcon,setHideIcon] = useState({1:true, 2:true, 3:true, 4:true, 5:true, 6:true});
  ...
  return (
    <div  className='flex flex-col mt-6 w-full h-fit lg:absolute lg:right-5 lg:top-13  lg:w-[60%] '>
        <div  className="flex flex-row justify-between items-center  ">
            ...
            <h1 className={`boxShadow2 flex relative z-1 flex-row justify-center items-center text-xl font-bold text-white/90 pl-5 pr-5 pb-2 pt-2 rounded-b-[50%] rounded-t-[50%] outline-none  ${theme===true? "bg-gradient-to-b from-violet-100 via-violet-600 to-violet-100" : "bg-zinc-700"}`}> Highlights</h1>
 ...
 <span>{pic}</span>
 <h2 className={`flex rounded-b-3xl value rounded-t-[100%] justify-center text-xl font-bold pb-1 pt-1 w-full text-white/90 ${theme===true? "bg-gradient-to-t from-violet-200 to-violet-600" : "bg-gradient-to-t from-zinc-600 to-zinc-700"}`}>{value}{math}</h2>
 </div>
         )  )}
