...
const uvIndex=weather.data.values.uvIndex;
  ...
  return (
    <div className='flex flex-col mt-6 w-full h-fit lg:absolute lg:right-5 lg:top-13  lg:w-[60%] '>
        ...
        <div className={` overflow-x-scroll grid grid-cols-2 lg:grid-cols-6 w-[98%] ml-1 h-fit ${theame===true? "bg-gradient-to-b from-violet-50 to-white boxshadow3 " : "bg-gradient-to-b from-zinc-800 to-white/10 boxshadow3dark"} gap-5 p-3 mt-4 rounded-2xl`}>
 ...
 <span>{pic}</span>
 <h2 className={`flex rounded-b-3xl value rounded-t-[100%] justify-center text-xl font-bold pb-1 pt-1 w-full text-white/90 ${theame===true? "bg-gradient-to-t from-violet-200 to-violet-600" : "bg-gradient-to-t from-zinc-600 to-zinc-700"}`}>{value}{math}</h2>
 </div>
         )  )}
