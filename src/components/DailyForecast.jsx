...
const DailyForecast = ({ selectDay }) => {
  ...
  return (
    <>
      {selectDay && (
        <div className="flex flex-col w-full h-fit lg:absolute lg:right-5 lg:top-[28%]  lg:w-[60%] ">
          <div
            className={`overflow-x-scroll grid grid-cols-2 lg:grid-cols-6 w-[99%] ml-0.5 h-fit justify-around ${
              theme === true
                ? "bg-gradient-to-b from-violet-50 to-white boxShadow3 "
                : "bg-gradient-to-b from-zinc-800 to-white/10 boxShadow3dark"
            }  gap-5 p-3 mt-1 rounded-tr-2xl rounded-br-2xl rounded-bl-2xl`}
          >
            {todayLineTwo.map((item) => (
              <div
                key={item.id}
                className={`flex flex-col boxShadow text-white/90 justify-between items-center ${
                  theme === true
                    ? "bg-violet-100 "
              ...
              >
                ...
                <span>{item.pic}</span>
                <h2
                  className={`flex flex-row rounded-b-3xl value rounded-t-[100%] justify-center text-2xl font-bold pb-1 pt-1 w-full text-white/90 ${
                    theme === true
                      ? "bg-gradient-to-t from-violet-200 to-violet-600"
